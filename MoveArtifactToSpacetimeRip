...
for(planet of df.getMyPlanets().filter(p => p.planetType === 2).filter(p => p.heldArtifactIds.length > 0)){
	...
	if(candidates_.length > 0){
		...
		ui.setArtifactSending(planet.heldArtifactIds[0], candidate.locationId)
		df.move(planet.locationId, candidate.locationId, energyNeeded, 0, planet.heldArtifactIds[0]);
		artCollec++;
